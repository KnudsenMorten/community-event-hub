...
/// for Otto's ops mode (REQUIREMENTS §133). The implementation answers ops questions —
/// speaker readiness / missing slides (§134), sponsor missing deliverables (§135),
/// master-class non-selections, and participation / task / attendee counts (§132 data) —
/// via CURATED, TYPED, READ-ONLY aggregate queries over the mirror + crew data. It is
/// NEVER free-form text-to-SQL and never writes.
///
/// <para><b>SECURITY — authorization at retrieval.</b> This provider is the organizer-only
/// grounding source. The gate lives in <see cref="OttoGroundingBuilder"/>, which calls it
...
/// </summary>
public interface IOttoOrganizerOpsProvider
{
    /// <summary>
    ...
    /// </summary>
    Task<IReadOnlyList<OttoGroundingSection>> GetOpsAggregatesAsync(

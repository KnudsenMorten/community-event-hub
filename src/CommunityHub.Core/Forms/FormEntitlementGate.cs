...
public static class FormEntitlementGate
{
        ...
        CommunityHubDbContext db, int eventId, int participantId, CancellationToken ct)
    {
        ...
        // §299 C5: main-day lunch is gated on the DERIVED presenting days (linked
        // sessions), so load them whenever a speaker hat is in play.
        var days = speaker is null
            ? null
            : await SpeakerDayScope.DaysForSpeakerAsync(db, eventId, participantId, ct);

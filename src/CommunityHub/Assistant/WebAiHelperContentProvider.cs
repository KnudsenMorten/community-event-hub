...
/// <summary>
/// Web implementation of <see cref="IAiHelperContentProvider"/>: returns the raw markdown
/// of a content-hub page from disk via <see cref="ContentMarkdownRenderer"/>. It does
/// NOT decide who may see a page — <see cref="AiHelperGroundingBuilder"/> only ever asks
/// for slugs the role is allowed to see (ContentPageRegistry.ForRole), so this is a
/// pure reader.
/// </summary>
public sealed class WebAiHelperContentProvider : IAiHelperContentProvider
{
    private readonly ContentMarkdownRenderer _renderer;

    public WebAiHelperContentProvider(ContentMarkdownRenderer renderer) => _renderer = renderer;

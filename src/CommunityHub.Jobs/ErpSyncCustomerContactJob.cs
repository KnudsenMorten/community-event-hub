...
/// </summary>
public sealed class ErpSyncCustomerContactJob
{
    ...
    private readonly ILogger<ErpSyncCustomerContactJob> _log;

    public ErpSyncCustomerContactJob(
        ...
        ILogger<ErpSyncCustomerContactJob> log,
    ...
    [Function("ErpSyncCustomerContactJob")]
    public async Task Run(
        // §510 — this is a BASE TICK, not the cadence. The real frequency is the operator-editable
        // JobRunState.MinIntervalMinutes, defaulting to JobCatalog's 30 so behaviour is unchanged.
        ...
        CancellationToken ct)
    {
        ...
        {
            _log.LogInformation(
                "ErpSyncCustomerContactJob: feature 'erp-webshop-reconcile' disabled for all active editions, skipped.");
            _activity?.ReportInactive(
                "The 'erp-webshop-reconcile' feature is off for EVERY active edition, so no customer "
                + "or contact is being reconciled between e-conomic and the webshop.");
            return;
        }

        if (!_sync.CanRun)
        {
            _log.LogInformation("ErpSyncCustomerContactJob: e-conomic / Company Manager not configured; skipped.");
            _activity?.ReportInactive(
                "e-conomic or Company Manager is not configured on this host, so nothing can be "
                + "reconciled.");
            return;
        }
        ...
        try
        {
            // §482b: the scheduled job still sweeps EVERY sponsor customer (no onlyCustomerNumber).
            r = await _sync.SyncAsync(ct: ct);
        }
        ...
        catch (Exception ex)
        {
            ...
            _log.LogError(ex,
                "ErpSyncCustomerContactJob: run FAILED (consecutive failure #{N}); alert {Alert}.",
            ...
            if (decision.ShouldAlert)
            {
                ...
                await _alerts.AlertAsync(
                    "Engine FAILED (2x in a row): ErpSyncCustomerContactJob [ELDK27]",
        ...
        _log.LogInformation(
            "ErpSyncCustomerContactJob: {Customers} customers, {Users} user(s) created, {Defaults} default(s) set, {Alerts} alert(s).",
